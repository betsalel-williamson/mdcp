...
2. NEVER modify tracked files, commit session work, or leave uncommitted edits while the current branch is `main` or `master`. Create the feature branch named in the plan from updated `main` before client shards, UI code, or tests. Verify with `git branch --show-current` (or equivalent) before the first edit. One branch per issue — do not mix unrelated UX work.
