...
describe('protocol version helpers', () => {
  it('abbreviates trailing zeros', () => {
    ...
    expect(abbreviateProtocolVersion('0.5.0.0')).toBe('0.5');
  ...
  it('expands to four parts', () => {
    ...
    expect(expandProtocolVersion('0.5')).toBe('0.5.0.0');
  ...
  it('parses llms index filenames', () => {
    ...
    expect(parseLlmsIndexFilename('mdcp.v0.5--draft.llms.txt')).toBe('0.5.0.0');
  ...
  it('detects draft filenames', () => {
    expect(isLlmsIndexDraftFilename('mdcp.v0.5--draft.llms.txt')).toBe(true);
    expect(isLlmsIndexDraftFilename('mdcp.v1--draft.llms.txt')).toBe(true);
    expect(isLlmsIndexDraftFilename('mdcp.v0.5.llms.txt')).toBe(false);
  });

  it('maps protocol version to npm release ref', () => {
    expect(protocolVersionToReleaseRef('0.5.0.0')).toBe('v0.5.0');
