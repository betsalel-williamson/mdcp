...
describe('cli smoke', () => {
  ...
  it('evaluate-doc-coverage emits JSON for missing docs in advisory mode', () => {
    const r = spawnSync(
      'node',
      [
        CLI,
    ...
    expect(r.status).toBe(0);
    const json = JSON.parse(r.stdout) as { status: string; docSurfaces: string[] };
    expect(json.status).toBe('missing_docs');
    expect(json.docSurfaces).toEqual(expect.arrayContaining(['features', 'client']));
  });

  it('evaluate-doc-coverage gate mode exits 1 on missing docs', () => {
    const r = spawnSync(
      'node',
      [
        CLI,
        ...
        'gate',
        '--changed',
        'packages/mdcp-core/src/index.ts',
      ],
      { encoding: 'utf-8', cwd: REPO_ROOT },
    );
    expect(r.status).toBe(1);
    const json = JSON.parse(r.stdout) as { status: string };
    expect(json.status).toBe('missing_docs');
  });

  it('evaluate-doc-coverage needs_clarification for opaque paths', () => {
    const r = spawnSync(
      'node',
      [CLI, 'evaluate-doc-coverage', '--docs-root', 'docs', '--changed', 'opaque.bin'],
      { encoding: 'utf-8', cwd: REPO_ROOT },
    );
    expect(r.status).toBe(0);
    const json = JSON.parse(r.stdout) as { status: string; questions: { id: string }[] };
