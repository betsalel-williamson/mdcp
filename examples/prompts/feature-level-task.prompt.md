...
**Setup:** Follow WORK_ITEM_LOOKUP above. Read `mdcp.v1.llms.txt` in the docs root when present — do not edit that file for repo-specific changes; use shards and `docs/extensions/` instead. Inspect the repository for scope, acceptance criteria, validation commands, and delivery conventions before editing. Treat acceptance criteria as the scope boundary — one feature or design at a time; do not expand into adjacent issues unless WORK_ITEM explicitly includes them.
