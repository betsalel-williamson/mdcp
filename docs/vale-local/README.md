...
`MDCP-Xref` warns on **Pandoc-style `{#…}` identifiers after headings** (for example a heading title followed by a brace-hash id). Authors should remove those markers; MDCP derives fragment targets from GFM heading text via GitHub-style slugs. The shipped `MDCP` style is separate: en-US prose cues when a numbered heading is _mentioned_ without a GFM link. See [Locale and language boundary](../features/design-constraints/locale-and-language.md).
