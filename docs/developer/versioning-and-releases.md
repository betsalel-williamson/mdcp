...
5. Merge the Version Packages PR. CI publishes bumped public packages (npm OIDC) and creates GitHub Releases. Private skill carriers are versioned in git only. The release job sets `HUSKY=0` for the version commit so local pre-commit hooks do not run on the bot commit (those hooks expect a built `dist/`).
