...
Branch before edit=plans MUST name the intended short-lived feature branch and link WORK_ITEM before “go”; create that branch from updated main before any tracked-file edit; NEVER modify tracked files, commit session work, or leave uncommitted edits on main/master; verify with git branch --show-current before the first edit; approved plan or verbal “go” that endorses stay on main does NOT authorize integration-branch edits — correct the delivery path first; explicit informed override (human knowingly opts out of branch + PR loop) → acknowledge once, proceed on integration branch
...
2. **Branch before edit** — plans MUST name the feature branch and link `WORK_ITEM` before “go”. Before the first tracked-file edit: pull updated `main`, create that branch, and verify with `git branch --show-current`. NEVER modify tracked files, commit session work, or leave uncommitted edits while on `main` or `master`. An approved plan or verbal “go” that endorses stay on main does **NOT** override this — correct the delivery path first. An explicit informed override (human knowingly opts out of the branch + PR loop) is the exception — acknowledge once, then proceed on the integration branch.
