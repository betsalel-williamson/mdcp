...
1. **Reads section files** — from link order in the manifest (`index.md` / `shards.md`). Use `compile.sectionsHeading` when the manifest has preamble links outside the ordered section list.
...
- **GFM only** — no Pandoc, wikilinks, or required `{#heading-ids}`.
