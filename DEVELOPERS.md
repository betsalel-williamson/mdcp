...
Classify the blast radius before merging. Full rules: [Automated updates protocol](#automated-updates-protocol).
...
- [Automated updates protocol](#automated-updates-protocol)
- [Agent Skill](#agent-skill-development)
- [.changeset/README.md](.changeset/README.md)

<!-- mdcp-shard: end docs/developer/versioning-and-releases.md -->

<!-- mdcp-shard: start docs/developer/automated-updates-protocol.md -->
...
Skills and npm packages version independently ([Versioning and releases](#versioning-and-releases)). `npx skills add betsalel-williamson/mdcp` copies [`skills/`](skills) from this repository’s **default branch tip**. npm consumers install `@bwilliamson/mdcp-*` only after a [Release](#publishing). Classify the blast radius before opening or merging an automated change.
...
1. Run `pnpm audit --prod`. Clean means the finding is not a published-package incident — prefer a Class A workspace override or ignore when no patched release exists ([Security-incident triage](#security-incident-triage)).
...
This repository does **not** run an AI assistant or `workflow_run` heal job in CI ([GitHub Actions security checklist](#github-actions-security-checklist)). Any future Cursor Automation or bot must classify the proposed diff against these classes before opening a pull request:
...
- [Security-incident triage](#security-incident-triage) — prod vs dev-only, overrides, unfixed advisories
- [Versioning and releases](#versioning-and-releases) — changesets and Dependabot merge gates
- [Agent Skill](docs/features/agent-skill.md) — consumer install and vendoring
- [Cursor Cloud environment](#cursor-cloud-environment) — audit-before-gates gotcha

<!-- mdcp-shard: end docs/developer/automated-updates-protocol.md -->
...
Automated PRs that only touch overrides, audit ignores, or other monorepo-dev paths are Class A — see [Automated updates protocol](#automated-updates-protocol).
...
- [Automated updates protocol](#automated-updates-protocol) — Class A / B / C merge gates
